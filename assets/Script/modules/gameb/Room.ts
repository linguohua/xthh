...
import { HandlerMsgSearchAck } from "./handlers/HandlerMsgSearchAck";
...
export const msgHandlers: { [key: number]: msgHandler } = {
    ...
    [msgCodeEnum.MSG_SEARCH_ACK]: HandlerMsgSearchAck.onMsg, //桌子进入托管
...
export class Room {
    ...
    public getNextPlayer(chairID: number): Player {
    ...
    public getBackPlayer(chairID: number): Player {
    ...
    public updateRoom(table: protoHH.casino.Itable): void {
        ...
        for (let i = 0; i < table.players.length; i++) {
            ...
            if (this.isMe(player.id.toString())) {
                ...
                if (p === null || p.userID !== player.id.toString()) {
