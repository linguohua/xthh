...
export namespace HandlerMsgActionOPAckA {
    const pong = (room: RoomInterfaceA, player: PlayerA, pAck: proto.casino_gdy.packet_sc_op_ack): void => {
        //从手牌移除
        for (let i = 0; i < 2; i++) {
    ...
    const kong2 = (room: RoomInterfaceA, t: number, player: PlayerA, pAck: proto.casino_gdy.packet_sc_op_ack, array: number[]): void => {
    ...
    const kong = (room: RoomInterfaceA, player: PlayerA, pAck: proto.casino_gdy.packet_sc_op_ack): number => {
