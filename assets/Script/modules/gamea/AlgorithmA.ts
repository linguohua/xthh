 ...
 * 客户端用的规则
 */
const DEF_GDY_MJ_MAX = 72;          // 牌总数
const DEF_GDY_MJ_NUM_MAX = 9;           // 基本牌最大数

const DEF_GDY_MJ_WAN = 1           // 万
const DEF_GDY_MJ_TIAO = 2;           // 条
const DEF_GDY_MJ_TONG = 3;           // 筒

const DEF_GDY_TYPE_TOTAL = 3;           // 牌类型总数

const DEF_GDY_MJ_MIN = 2;           // 手上牌剩余最少数量

const DEF_GDY_NAIZI_MAX = 4;           // 最大赖子数量
const DEF_GDY_NAIZI_ERR = 99;          // 需要赖子数量的错误

const DEF_GDY_PENG = 1;
const DEF_GDY_GANG = 2;
const DEF_GDY_HU = 3;
const DEF_GDY_ZIMO = 4;
const DEF_GDY_CHAOTIAN = 5;
const DEF_GDY_BUZHUOCHONG = 6;           // 不捉铳
const DEF_GDY_QIANGXIAO = 7;
...
export class AlgorithmA {
    ...
    private m_nMahjongTotal: number = DEF_GDY_MJ_MAX; // 自己记录牌总数
    ...
    private init(): void {
        ...
        let index = 0
        this.m_sMahjongs = [];
        for (let i = 0; i < DEF_GDY_MJ_NUM_MAX; i++) {
            for (let j = 0; j < 4; j++) {
                this.m_sMahjongs[index] = DEF_GDY_MJ_TONG * 10 + i;
                this.m_sMahjongs[index + 1] = DEF_GDY_MJ_TIAO * 10 + i;
                this.m_sMahjongs[index + 2] = DEF_GDY_MJ_WAN * 10 + i
                index = index + 3;
            }
        }
    }
    ...
    public randomLaiZi(): void {
        const random_num = Math.floor(Math.random() * DEF_GDY_MJ_MAX - 1);
        // 先随机翻牌
    ...
    public randomSort(): void {
        ...
        this.m_nReverseIndex = DEF_GDY_MJ_MAX - 1;
        // 判断是否用配置牌
        ...
        let index = 0;
        let random_num = 0;
        for (let i = 0; i < DEF_GDY_MJ_MAX; i++) {
            //假如不适用配置牌那么随机调换牌顺序
            if (bWithCards) {
                this.m_sMahjongs[i] = DEF_WITH_CARDS[i + 1];
            } else {
                index = this.m_sMahjongs[i];
                random_num = Math.floor(Math.random() * DEF_GDY_MJ_MAX - 1);
    ...
    //设置／获取／减少当前麻将牌总数
    public mahjongTotal_set(total: number = DEF_GDY_MJ_MAX): void {
    ...
    public canHuPai_def(mahjongs: number[], isHeiMo: boolean): number[] {
        const sVecHuPai: number[] = [];
        ...
        if (isHeiMo && array.sVecLai.length === 1) {
            array.sVecPai.push(array.sVecLai[0]);
            array.sVecLai = [];
        }
        this.defMahjongSort_stb(array.sVecPai);
    ...
    //会去除杠的牌,判断能否胡牌(递归方式)
    public canHuPai_defEX(mahjongs: number[], isHeiMo: boolean): ArrayClass_c {
        ...
        if (isHeiMo && a.sVecLai.length === 1) {
            a.sVecPai.push(a.sVecLai[0]);
            a.sVecLai = [];
        }
        let bHuPai = false;
    ...
    public canGangPai_withAll(v_mahjongs: number[], s_mahjongs: proto.casino_gdy.packet_sc_op_ack[], f_mahjongs: number[]): number[] {
    ...
    public haveGang_WithMe(tilesHand: number[], melds: proto.casino_gdy.packet_sc_op_ack[], notKongs: number[], mahjong: number): number[] {
