 ...
 */
// getSystemInfo
declare interface getSystemInfoRes {
  brand: string;
  model: string;
  pixelRatio: any;
  screenWidth: number;
...
interface midasPaymentOpts extends WxApiCallback<getWeRunDataRes> {
  mode: string;
  env: number;
  offerId: string;
  currencyType: string;
...
interface OpenInterfaceAPIs {
  ...
  getLaunchOptionsSync: () => ApplaunchShowOpts;//TODO
	 ...
	 * 发起米大师支付
	 */
  requestMidasPayment(midasPaymentOpts: midasPaymentOpts): void;
}
...
interface ALDAPIs {
  aldSendEvent(eventName: string, paramMap?: string2AnyMap): void;
}

/**
 * 广告相关接口
 */
interface AdvertisementAPIS {
