...
export namespace LocalStrings {
        ...
        const localStrings: { [key: string]: string } = {
                ...
                [`approvalInformation`]: "粤网文 [2016] 5856-1386号 [许可证 [粤B2-20160210] 新广初审[2017] 9751号] ISBN 978-7-7979-4213-3 北京科海电子出版社",
...
重要须知：
                ...
                [`agreeText1`]: `1.1 本协议：指本协议正文、《用户服务协议》、《用户隐私政策》、游戏规则及其修订版本。上述内容一经正式发布，即为本协议不可分割的组成部分。本协议同时还包括文化部根据《网络游戏管理暂行办法》（文化部令第49号）制定的《网络游戏服务格式化协议必备条款》。1.2 游戏规则：指服务提供方（甲方）不时发布并修订的关于我方的用户守则、玩家条例、游戏公告、提示及通知等内容。1.3 甲方：指向您提供服务的《众游仙桃晃晃》，在本协议中简称为“甲方”。1.4 甲方游戏：指由甲方负责运营的游戏的统称，包括计算机客户端游戏、网页游戏、HTML5游戏（H5游戏）、移动终端游戏、电视端游戏以及其他形式的游戏；我方可能以软件形式提供，这种情况下，我方还包括该相关软件及相关文档。1.5 甲方服务：指甲方向您提供的与游戏相关的各项在线运营服务。1.6 乙方：又称“玩家”或“用户”，指被授权使用我方及其服务的自然人。1.7 游戏数据：指您在使用我方过程中产生的被服务器记录的各种数据，包括但不限于角色数据、虚拟物品数据、行为日志、购买日志等等数据。二、【游戏账号】`,
